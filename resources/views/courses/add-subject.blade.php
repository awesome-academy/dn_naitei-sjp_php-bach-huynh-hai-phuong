<x-layout.admin-panel title="{{ __('course_subject.add') }}">
    <x-ui.card>
        ...
        <x-ui.card.content>
            @if (count($formatedAvailableSubjects) !== 0)
                <form action="{{ route('courses.subject.add', $course->id) }}" method="POST" class="grid gap-4">
                    ...
                    <div class="grid gap-2">
                        ...
                        <x-ui.select name="subject_id" id="subject_id" :items="$formatedAvailableSubjects"
            ...
            @else
                <p class="text-sm text-muted-foreground">{{ __('course_subject.no_subject_to_add') }}</p>
            @endif
        </x-ui.card.content>
    </x-ui.card>
